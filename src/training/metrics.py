...
def calculate_ssim(img1, img2, max_value=1.0):
    ...
    if len(img1.shape) == 4:
        ...
        for i in range(img1.shape[0]):
            ...
            # win_size debe ser <= min(H, W) y ser impar (mínimo 3)
            min_dim = min(img1_i.shape[0], img1_i.shape[1])
            if min_dim < 3:
                ssim_values.append(1.0)  # Patch demasiado pequeño
                continue
            win_size = min(7, min_dim if min_dim % 2 == 1 else min_dim - 1)

            ssim = compare_ssim(
                img1_i, img2_i, data_range=max_value, channel_axis=2, win_size=win_size
            )
            ssim_values.append(ssim)

        return np.mean(ssim_values) if ssim_values else 1.0
