...
class AblationStudy:
    ...
    def print_summary(self):
        ...
        # Baseline
        baseline = df[df['variant'] == 'baseline'].iloc[0] if 'baseline' in df['variant'].values else None
        ...
        if baseline is not None:
            ...
            for _, row in df.iterrows():
                if row['variant'] != 'baseline':
                    ...
                    print(f"{row['variant']:30s} | PSNR: +{psnr_gain:+.2f} dB | SSIM: +{ssim_gain:+.4f}")


def main():
    parser = argparse.ArgumentParser(description='Run Ablation Study')
    parser.add_argument('--base-config', type=str, default='config/satellite_x4.yaml',
                       help='Base configuration')
    parser.add_argument('--val-dir', type=str, default='data/satellite/dataset/val',
