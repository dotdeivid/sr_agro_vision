...
def evaluate_ensemble(model_paths, val_dir, weights=None):
    ...
    for result in individual_results:
        name = result['name']
        m = result['metrics']
        print(f"{name:<30} | {m['psnr']:>8.2f} | {m['ssim']:>8.4f} | {m['ndvi_mae']:>10.4f}")
    ...
    print(f"{'-'*30}-+-{'-'*8}-+-{'-'*8}-+-{'-'*10}")
    print(f"{'Ensemble (estimado)':<30} | {avg_psnr:>8.2f} | {avg_ssim:>8.4f} | {avg_ndvi:>10.4f}")
