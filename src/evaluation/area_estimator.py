...
class AreaEstimator:
    ...
    def compare_areas(self, sr_img, hr_img, ndvi_threshold=0.3):
        ...
        error_percent = (error_ha / area_hr['area_ha'] * 100) if area_hr['area_ha'] > 0 else 0
        ...
        dice = 2 * intersection / (np.sum(mask_sr) + np.sum(mask_hr)) if (np.sum(mask_sr) + np.sum(mask_hr)) > 0 else 0
    ...
    def _save_results(self, results, output_dir):
        ...
        summary = {
            ...
            'mean_error_percent': float(np.mean([abs(r['error_percent']) for r in results])),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-dir', type=str, default='results/phase4/area')
    parser.add_argument('--gsd', type=float, default=10.0, help='Ground sample distance (meters)')
    parser.add_argument('--ndvi-threshold', type=float, default=0.3)
    
    args = parser.parse_args()
    
    estimator = AreaEstimator(gsd_meters=args.gsd)
    results = estimator.evaluate_batch(args.sr_dir, args.hr_dir, args.output_dir, args.ndvi_threshold)
