"""
Generador de informe final para evaluación Fase 4
...
class ReportGenerator:
    ...
    def load_all_results(self):
        ...
        metrics_file = self.results_dir / 'metrics' / 'agricultural_metrics_summary.json'
        ...
        # Classification
        class_file = self.results_dir / 'classifications' / 'classification_results.json'
    ...
    def generate_executive_summary(self):
        ...
        if 'agricultural_metrics' in self.report_data:
            ndvi_mae = self.report_data['agricultural_metrics'].get('NDVI', {}).get('MAE_mean', 0)
            summary.append(f"- **NDVI MAE**: {ndvi_mae:.4f}\n")
        
        # Classification
        if 'classification' in self.report_data:
            acc_improvement = self.report_data['classification'].get('improvement_vs_lr', 0) * 100
            summary.append(f"- **Mejora en Clasificación (SR vs LR)**: +{acc_improvement:.2f}%\n")
        
        # Area estimation
        if 'area_estimation' in self.report_data:
            area_error = self.report_data['area_estimation'].get('mean_error_percent', 0)
            summary.append(f"- **Error en Estimación de Área**: {area_error:.2f}%\n")
        
        # Economic
        if 'economic' in self.report_data:
            roi = self.report_data['economic'].get('roi_analysis', {}).get('roi_percent', 0)
            summary.append(f"- **ROI**: {roi:.1f}%\n")
        
        summary.append("\n### Conclusiones\n\n")
        summary.append("La super-resolución (SR) demuestra ser una alternativa viable y económicamente rentable ")
        summary.append("para aplicaciones agrícolas, con mejoras significativas en:\n\n")
    ...
    def generate_markdown_report(self, output_path):
        ...
        if 'agricultural_metrics' in self.report_data:
            ...
            for idx in ['NDVI', 'EVI', 'SAVI']:
                if idx in self.report_data['agricultural_metrics']:
                    ...
                    report.append(f"#### {idx}\n\n")
                    report.append(f"- **MAE**: {data.get('MAE_mean', 0):.4f} ± {data.get('MAE_std', 0):.4f}\n")
                    report.append(f"- **RMSE**: {data.get('RMSE_mean', 0):.4f} ± {data.get('RMSE_std', 0):.4f}\n")
                    report.append(f"- **R²**: {data.get('R2_mean', 0):.4f} ± {data.get('R2_std', 0):.4f}\n")
                    report.append(f"- **Pearson r**: {data.get('Pearson_r_mean', 0):.4f}\n\n")
        
        # 2. Classification
        if 'classification' in self.report_data:
            report.append("## 2. Clasificación de Cultivos\n\n")
            cls_data = self.report_data['classification']
            report.append(f"- **Accuracy LR**: {cls_data.get('LR_accuracy_mean', 0):.4f}\n")
            report.append(f"- **Accuracy SR**: {cls_data.get('SR_accuracy_mean', 0):.4f}\n")
            report.append(f"- **Accuracy HR**: {cls_data.get('HR_accuracy_mean', 0):.4f}\n")
            report.append(f"- **Mejora SR vs LR**: +{cls_data.get('improvement_vs_lr', 0)*100:.2f}%\n\n")
        
        # 3. Area Estimation
        if 'area_estimation' in self.report_data:
            report.append("## 3. Estimación de Área Cultivada\n\n")
            area_data = self.report_data['area_estimation']
            report.append(f"- **Error Promedio**: {area_data.get('mean_error_percent', 0):.2f}%\n")
            report.append(f"- **IoU**: {area_data.get('mean_iou', 0):.4f}\n")
            report.append(f"- **Dice Coefficient**: {area_data.get('mean_dice', 0):.4f}\n\n")
        ...
        if 'temporal' in self.report_data:
            ...
            report.append(f"- **Correlación Temporal**: {temp_data.get('temporal_correlation', 0):.4f}\n")
            report.append(f"- **MAE Temporal**: {temp_data.get('temporal_mae', 0):.4f}\n")
            report.append(f"- **Trend Agreement**: {temp_data.get('trend_agreement', 0):.4f}\n\n")
        ...
        if 'economic' in self.report_data:
            ...
            report.append(f"- **Sentinel-2 + SR**: ${comp.get('sentinel2_sr', {}).get('total', 0):.2f}\n")
            report.append(f"- **Planet 3m**: ${comp.get('planet_3m', {}).get('total', 0):.2f}\n")
            report.append(f"- **WorldView 50cm**: ${comp.get('worldview_50cm', {}).get('total', 0):.2f}\n\n")
            
            report.append("### ROI\n\n")
            roi_data = econ_data.get('roi_analysis', {})
            report.append(f"- **ROI**: {roi_data.get('roi_percent', 0):.1f}%\n")
            report.append(f"- **Payback Period**: {roi_data.get('payback_months', 0):.1f} meses\n\n")
        
        # 6. Use Cases
        if 'use_cases' in self.report_data:
            report.append("## 6. Casos de Uso\n\n")
            uc_data = self.report_data['use_cases']
            report.append(f"- **Estrés Hídrico (Accuracy)**: {uc_data.get('water_stress_accuracy_mean', 0):.4f}\n")
            report.append(f"- **Salud de Cultivos (Correlación)**: {uc_data.get('crop_health_correlation_mean', 0):.4f}\n\n")
        
        # Conclusions
        report.append("\n---\n\n")
        report.append("# CONCLUSIONES\n\n")
        report.append("1. **Viabilidad Técnica**: SR demuestra alta precisión en todas las métricas agrícolas evaluadas.\n\n")
        report.append("2. **Viabilidad Económica**: SR presenta ahorro significativo vs imágenes HR comerciales.\n\n")
        report.append("3. **Aplicabilidad**: Validado en casos de uso reales (estrés hídrico, clasificación, áreas).\n\n")
        report.append("4. **Recomendación**: SR es una solución viable para agricultura de precisión con presupuesto limitado.\n\n")
    ...
    def generate_latex_report(self, output_path):
        ...
        latex.append("\\title{Evaluación Específica del Dominio Agrícola\\\\Super-Resolución en Imágenes Satelitales}\n")
        latex.append("\\author{}\n")
        latex.append(f"\\date{{{datetime.now().strftime('%B %Y')}}}\n\n")
        latex.append("\\begin{document}\n\n")
        latex.append("\\maketitle\n\n")
        latex.append("\\section{Resumen Ejecutivo}\n\n")
        latex.append("Este informe presenta la evaluación exhaustiva de super-resolución (SR) aplicada a imágenes satelitales para agricultura de precisión.\n\n")
        ...
        if 'agricultural_metrics' in self.report_data:
            ...
            for idx in ['NDVI', 'EVI', 'SAVI']:
                if idx in self.report_data['agricultural_metrics']:
                    data = self.report_data['agricultural_metrics'][idx]
                    latex.append(f"{idx} & {data.get('MAE_mean', 0):.4f} & {data.get('RMSE_mean', 0):.4f} & {data.get('R2_mean', 0):.4f} \\\\\\\\\n")
        ...
        latex.append("SR es viable técnica y económicamente para aplicaciones agrícolas.\n\n")
    ...
    def generate_full_report(self, output_dir):
        ...
        # Generate reports
        self.generate_markdown_report(output_dir / 'PHASE4_FINAL_REPORT.md')
        self.generate_latex_report(output_dir / 'phase4_report.tex')
        ...
        print(f"Archivos:")
        print(f"   - PHASE4_FINAL_REPORT.md (Markdown)")
        print(f"   - phase4_report.tex (LaTeX para tesis)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Generate Phase 4 Final Report')
    parser.add_argument('--results-dir', type=str, default='results/phase4',
                       help='Directory with all evaluation results')
    parser.add_argument('--output', type=str, default='results/phase4/report',
