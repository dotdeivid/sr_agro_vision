...
class EconomicAnalyzer:
    ...
    def calculate_sr_cost(self, n_images, area_km2=100):
        ...
        """
        processing_time = n_images * self.costs['sr_processing']['processing_time_per_image']
        gpu_cost = processing_time * self.costs['sr_processing']['gpu_hour']
        
        # Storage (asumiendo 100MB por imagen SR)
        storage_gb = n_images * 0.1
        storage_cost_monthly = storage_gb * self.costs['sr_processing']['storage_per_gb']
    ...
    def calculate_hr_satellite_cost(self, n_images, area_km2=100, resolution_type='planet_3m'):
        """
    ...
    def compare_alternatives(self, n_images, area_km2=100):
        ...
        # WorldView 50cm
        worldview_cost = self.calculate_hr_satellite_cost(n_images, area_km2, 'worldview_50cm')
        ...
        comparison['savings_percent_planet'] = (comparison['savings_vs_planet'] / planet_cost['total'] * 100) if planet_cost['total'] > 0 else 0
        comparison['savings_percent_worldview'] = (comparison['savings_vs_worldview'] / worldview_cost['total'] * 100) if worldview_cost['total'] > 0 else 0
    ...
    def calculate_roi(self, n_images, area_km2, yield_improvement_percent=5):
        ...
        payback_months = (sr_cost / value_improvement * 12) if value_improvement > 0 else 999
    ...
    def visualize_cost_comparison(self, n_images, area_km2, output_path):
        ...
        for i, saving in enumerate(savings_values):
            axes[1].text(saving, i, f'{saving:.1f}%', ha='left', va='center', fontsize=12)
    ...
    def generate_report(self, n_images, area_km2, output_dir):
        ...
        # Visualización
        self.visualize_cost_comparison(n_images, area_km2, output_dir / 'cost_comparison.png')
        ...
        print(f"\nAhorro con SR:")
        print(f"   vs Planet: ${comparison['savings_vs_planet']:.2f} ({comparison['savings_percent_planet']:.1f}%)")
        print(f"   vs WorldView: ${comparison['savings_vs_worldview']:.2f} ({comparison['savings_percent_worldview']:.1f}%)")
        print(f"\nROI:")
...
if __name__ == "__main__":
    ...
    report = analyzer.generate_report(n_images=12, area_km2=100, output_dir='results/phase4/economic')
