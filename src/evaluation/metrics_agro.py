...
class AgriculturalMetrics:
    ...
    def classify_vegetation_health(
        ...
        report = classification_report(
            hr_flat, sr_flat, target_names=class_names, output_dict=True
        )
...
def evaluate_batch(sr_dir, hr_dir, output_dir, dataset_type="multiespectral"):
    """
    ...
    # Verificar si es RGB - no calcular métricas agrícolas
    if dataset_type == "rgb":
        print("\n⚠️  Dataset tipo RGB detectado")
        print(
            "   Las métricas agrícolas (NDVI, EVI, SAVI) requieren datos multiespectrales"
        )
        print("   Para evaluar métricas RGB, usa el módulo de evaluación estándar\n")

        # Crear archivo vacío indicando que no hay métricas
        summary = {
            "note": "Agricultural metrics not applicable for RGB datasets",
            "dataset_type": "rgb",
            "requires": "multispectral data with NIR band",
        }
    ...
    print(f"\n📊 Evaluando {len(sr_files)} imágenes multiespectrales...")
    ...
    for sr_file in sr_files:
        ...
        # Verificar que tenga al menos 4 canales
        if sr_img.shape[0] < 4 or hr_img.shape[0] < 4:
            print(
                f"⚠️  Saltando {sr_file.name}: No tiene banda NIR (canales: {sr_img.shape[0]})"
            )
            continue
...
def main(args=None):
    ...
    if args is None:
        ...
        parser.add_argument(
            "--sr-dir", type=str, required=True, help="Directory with SR images"
        )
        parser.add_argument(
            "--hr-dir", type=str, required=True, help="Directory with HR ground truth"
        )
        parser.add_argument(
            "--output-dir",
            type=str,
            default="outputs/reports/metrics",
