...
class WaterStressDetector:
    ...
    def __init__(self):
        # Umbrales NDVI que definen 4 clases sin brechas:
        #   Severe:   NDVI < 0.3
        #   Moderate: 0.3 ≤ NDVI < 0.5
        #   Mild:     0.5 ≤ NDVI < 0.7
        #   Healthy:  NDVI ≥ 0.7
        self.stress_thresholds = {
            "severe": 0.3,
            "moderate": 0.5,
            "healthy": 0.7,  # Mild cubre [moderate, healthy) implícitamente
        }
    ...
    def detect_stress(self, image):
        ...
        # Clasificar estrés
        # Cobertura completa del espacio NDVI sin brechas:
        #   Severe   → NDVI < 0.3
        #   Moderate → 0.3 ≤ NDVI < 0.5
        #   Mild     → 0.5 ≤ NDVI < 0.7  (incluye el rango 0.6-0.7 que antes quedaba sin clasificar)
        #   Healthy  → NDVI ≥ 0.7
        stress_map = np.zeros_like(ndvi, dtype=np.uint8)
        stress_map[ndvi < self.stress_thresholds["severe"]] = 3  # Severo
        stress_map[
            (ndvi >= self.stress_thresholds["severe"])
            & (ndvi < self.stress_thresholds["moderate"])
        ] = 2  # Moderado
        stress_map[
            (ndvi >= self.stress_thresholds["moderate"])
            & (ndvi < self.stress_thresholds["healthy"])
        ] = 1  # Leve
    ...
    def compare_detection(self, sr_img, hr_img):
        ...
        all_names = ["Healthy", "Mild", "Moderate", "Severe"]
        # Solo incluir clases presentes en alguno de los dos mapas
        present_labels = sorted(
            set(stress_hr.flatten().tolist()) | set(stress_sr.flatten().tolist())
        )
        present_names = [all_names[i] for i in present_labels if i < len(all_names)]
...
def evaluate_use_cases(sr_dir, hr_dir, output_dir):
    ...
    results = {"water_stress": [], "crop_health": []}
    ...
    for sr_file in sr_files:
        ...
        health_result = {
            ...
            "health_correlation": np.corrcoef(health_sr.flatten(), health_hr.flatten())[
                0, 1
            ],
        }
